...
class BaselineDataset(torch.utils.data.Dataset):
    ...
    def _baseline_tkn(self, df:pd.DataFrame) -> np.ndarray:
        df['subject_entity'] = df['subject_entity'].apply(lambda x: literal_eval(x)['word'])
        df['object_entity'] = df['object_entity'].apply(lambda x: literal_eval(x)['word'])
        df['input'] = df['subject_entity'] + ['[SEP]'] + df['object_entity'] + ['[SEP]'] + df['sentence']
        
        return df['input'].to_numpy()
