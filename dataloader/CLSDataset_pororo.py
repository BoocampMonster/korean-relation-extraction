...
from utils import pre_marker_pororo
import pandas as pd
import einops as ein
import torch

class CLSDataset_pororo(torch.utils.data.Dataset):
    """_summary_
    ...
    def __init__(self, 
        ...
        if self.mode == 'train':
            self.sentence_array, entity_hint, tokenizer, self.target_array = self._load_data(data, tokenizer)
        else:
            self.sentence_array, entity_hint, tokenizer = self._load_data(data, tokenizer)
    ...
    def _load_data(self, data:pd.DataFrame, tokenizer):
        ...
        sentence, entity_hint, tokenizer = getattr(pre_marker_pororo, self.entity_marker_mode)(data, tokenizer)
